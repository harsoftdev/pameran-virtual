...
export const createBoundingBoxes = (objects) => {
	// objects will be either archives or walls that we pass in from main.js
	...
	objects.forEach((object) => {
		...
		object.BoundingBox.setFromObject(object); // set the bounding box to the object (archive or wall)
	});
};
...
// Checking for both THREE.Group and arrays is because the `createWalls` function is returning a THREE.Group while `createArchives` is returning an array.
...
// However, createArchives returns an array, not a Group. When you call createBoundingBoxes(archives), the objects parameter of createBoundingBoxes is already an array, so you don't need to do anything.
